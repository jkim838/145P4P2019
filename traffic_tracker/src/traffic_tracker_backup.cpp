    ...
    if(frame_count == 1){
      ...
      for(size_t i = 0; i < vehicles.size(); i++){
        ...
        for(size_t j = 0; j < prev_vehicles.size(); j++){
          ...
          export_csv << "----------\n";
          export_csv << "Using threshold value X: "<< current_x_dimension/3 << " and Y: " << current_y_dimension/3 << "\n";
          export_csv.close();
          /***END DEBUG MESSAGE***/

          // TODO: DYNAMICALLY ADJUST THE THRESHOLD VALUE PER DIFFERENT SIZE OF BBOX...
          if(abs_x_diff < (current_x_dimension/3) && abs_y_diff < (current_y_dimension/3)){
